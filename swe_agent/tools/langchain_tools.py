"""
LangChain Tool wrappers for filesystem operations in swe_agent.tools.
...
Each tool validates input via Pydantic models and returns a JSON string
constructed from Pydantic output models.

Use get_default_tools(workspace, config) to obtain a ready-to-use list
of LangChain tool objects bound to a specific workspace where applicable.
"""
...
# Local functional implementations
from .fs import read_file as _read_file
from .fs import write_file as _write_file
from .fs import list_files as _list_files
from .fs import search_text as _search_text
from .patch import apply_unified_diff as _apply_unified_diff
from .patch import create_file as _create_file
...
def get_default_tools(workspace: Optional[str] = None, config: Optional[Dict[str, Any]] = None) -> List[Any]:
    """Return the default set of LangChain tools for filesystem operations.

    All tools are bound to the given workspace and config via closures, avoiding
    reliance on global state.

    Args:
        workspace: Filesystem workspace root (absolute or relative path string).
        config: Optional dict with keys: max_read_bytes, ignore_globs, allow_delete, binary_threshold.

    Returns:
        A list of LangChain tool objects ready to be passed to an agent.
    """
    # Create a local SweAgentConfig instance
    from ..config import SweAgentConfig, DEFAULT_IGNORE
    ...
    # Define tool implementations using closures
