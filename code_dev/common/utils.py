...
def append_log_md(section: str, text: str, md_path=P.logs + "/interaction_log.md"):
    cur = read(md_path)
    cur += f"\n\n## {section}\n{text.strip()}\n"
    write(md_path, cur or f"# Внешний контекст проекта\n\n## {section}\n{text.strip()}\n")

def append_log_json(event: Dict[str,Any], json_path=P.logs + "/interaction_log.json"):
...
def build_repo_snapshot_per_root(
    ...
    """Возвращает строку с контекстом: список файлов + содержимое части из них."""
    """
    roots_with_patterns: список кортежей (root_dir, includes_patterns_relative_to_root)
    Пример: [("out/code/backend", ["**/*.py","**/*.json"]), ("out/docs", ["SD_openapi.yaml","BRD.json"])]
    """
    lines = []
    total_chars = 0
    collected = 0
    ...
    for root, includes in roots_with_patterns:
        ...
        for f in files:
            ...
            rel = str(f.relative_to(base)).replace("\\","/")  # <-- rel вычисляем тут, ДО использования
            block = f"\n=== {rel} ===\n{txt}\n"
            need = len(block)
...
def build_dev_context(agent: str) -> str:
    """
    agent: 'BE' | 'FE' | 'OPS'
    Собираем полезный контекст из out/.
    """
    ensure_dirs()

    # Паттерны включения ДОЛЖНЫ быть относительны корням ниже!
    if agent == "BE":
        roots = [
        ...
        roots = [
            ...
            (P.code_be, ["**/*.json","**/*.yaml","**/*.yml"]),  # чтобы видеть конфиг и контрактные штуки
