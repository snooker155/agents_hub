...
Security model
--------------
• The Docker socket is NEVER mounted inside containers.
...
• All containers share the "agents-hub" bridge network and can
  ...
  Docker daemon.
• The host ContainerManager is the sole authority for starting
  and stopping containers.

Volume mount
------------
The project root is mounted read-write at /app inside every
container.  This means state files, log files, and workspace
directories are all shared with the host in real time.
...
------------------
  Generated Dockerfiles are saved to:
    agents/state/dockerfiles/<agent_id>.Dockerfile
...
DOCKERFILE_DIR = PROJECT_ROOT / "agents" / "state" / "dockerfiles"
...
_PASS_EXACT = {
    "DEFAULT_PROVIDER", "WORKSPACE_ROOT", "TASKS_FILE",
}
...
def get_or_create_network() -> str:
    ...
    if NETWORK_NAME in (result.stdout or "").splitlines():
        return NETWORK_NAME
...
def generate_dockerfile(
    ...
    includes --http-port so the node_runner starts the HTTP server.
    """
...
#     -f agents/state/dockerfiles/{agent_id}.Dockerfile .
#
...
{expose_line}
CMD ["python", "-m", "agents.node_runner", "--agent-id", "{agent_id}"{http_cmd}]
"""
...
def build_image(
    ...
    Generates the Dockerfile, saves it to agents/state/dockerfiles/,
...
def image_tag_for_agent(agent_id: str) -> str:
    ...
    if image_exists(BASE_IMAGE):
        return BASE_IMAGE
    # Fall back to configured global image
    return os.environ.get("AGENT_DOCKER_IMAGE", BASE_IMAGE)
...
def start_container(
    ...
    Mount layout inside container:
      /app/agents/state  ← agents/state  (node records, logs, dockerfiles)
    ...
    # Build path mapping: host path → container path
    state_dir = str(PROJECT_ROOT / "agents" / "state")
    tasks_dir = str(PROJECT_ROOT / "tasks")
    path_mappings = {
        state_dir: "/app/agents/state",
        tasks_dir: "/app/tasks",
    }
    if workspace:
    ...
    docker_cmd = [
        ...
        "-v", f"{state_dir}:/app/agents/state",
        "-v", f"{tasks_dir}:/app/tasks",
        "-w", "/app",
    ]
    ...
    if workspace:
        ...
        docker_cmd.extend(["-v", f"{ws_abs}:/workspace"])
    ...
    # Extra user-configured docker flags
    extra = extra_args or os.environ.get("AGENT_DOCKER_EXTRA_ARGS", "").strip()
    if extra:
